...
export async function createOrder(formData: FormData) {
  ...
  const supplier_cost_price = product.price; // Preço que o fornecedor definiu

  // 3. Cálculos Financeiros (Baseados no seu blueprint)
  const logistics_cost = 20; // Exemplo fixo de R20 por entrega
  const total_revenue = seller_selling_price * quantity;
  const total_cost = (supplier_cost_price * quantity) + logistics_cost;

  // Taxas do Seller (5% do preço de venda + R10)
  const seller_fee = (total_revenue * 0.05) + (10 * quantity);
  // Taxa do Supplier (5% do preço de custo + R10) - (Supondo que a taxa é por venda)
  ...
  // 5. Registrar no Financial Ledger (Já que temos o order.id)
  const ledgerEntries = [
    {
      order_id: order.id,
    ...
    },
    {
      ...
      amount: -seller_fee, // Valor negativo pois é uma dedução
    },
    {
      order_id: order.id,
      user_id: user.id, // Seller
