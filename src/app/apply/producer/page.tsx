  ...
  return (
    <div className="min-h-screen bg-secondary flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-2xl border border-light-border shadow-sm p-8">
        ...
        <h2 className="text-xl font-semibold text-light-text text-center mb-1">Producer Application</h2>
        <p className="text-center text-light-muted text-sm mb-6">Tell us about your production.</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="text-xs font-medium text-light-muted">Company / Brand Name</label><input name="businessName" required className="w-full px-4 py-2 bg-secondary/50 border border-light-border rounded-lg" /></div>
          <div><label className="text-xs font-medium text-light-muted">Business Type</label><select name="businessType" className="w-full px-4 py-2 bg-secondary/50 border border-light-border rounded-lg"><option>Sole Proprietor</option><option>Company (Pty) Ltd</option><option>Cooperative</option><option>Other</option></select></div>
          ...
          <div><label className="text-xs font-medium text-light-muted">Business / Social Media Page</label><input name="socialMedia" className="w-full px-4 py-2 bg-secondary/50 border border-light-border rounded-lg" /></div>
          
          <h3 className="text-sm font-bold text-light-text border-b pb-2 mt-4">Production</h3>
          <div><label className="text-xs font-medium text-light-muted">What do you produce?</label><textarea name="productionDescription" className="w-full px-4 py-2 bg-secondary/50 border border-light-border rounded-lg h-20" required /></div>
          <div><label className="text-xs font-medium text-light-muted">Production Categories (Press Enter to add)</label><div className="flex flex-wrap gap-2 p-2 border border-light-border rounded-lg bg-secondary/50"><input type="text" placeholder="e.g. Clothing" onKeyDown={addCategory} className="flex-1 bg-transparent outline-none text-sm" />{categories.map(c => <span key={c} className="px-2 py-1 bg-primary/10 text-primary text-xs rounded-full">{c}</span>)}</div></div>
          <div><label className="text-xs font-medium text-light-muted">Approximate Production Capacity</label><select name="productionCapacity" className="w-full px-4 py-2 bg-secondary/50 border border-light-border rounded-lg"><option>Small (1-100 units/month)</option><option>Medium (101-1000 units/month)</option><option>Large (1000+ units/month)</option></select></div>
